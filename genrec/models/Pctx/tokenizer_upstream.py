...
class SASRecTokenizer(AbstractTokenizer):
    ...
    def _generate_interaction2personalized_semantic_emb_pkl(self,item_seqs,calculate_cold):
        ...
        # 2) find all appered items
            ...
            if missing_items:
                self.log(f"[CHECK] The number of cold-start items: {len(missing_items)}, displaying [:50]: {missing_items[:50]}{'...' if len(missing_items) > 50 else ''}")
            else:
                self.log("[CHECK] All items appeared at least once (there are no cold-start items).")
