...
class PctxTokenizer(AbstractTokenizer):
    ...
    def __init__(self, config: dict, dataset: AbstractDataset):
        ...
        # merge duplicated in paper is also named as merge conflict in code
        # merge infrequent in paper is also named as merge low frequency in code
    ...
    def compare_cluster_centers_sum(self, dict1, dict2):
        ...
        if self.accelerator.is_main_process:
            ...
            self.log(f'When we use merging duplicated, the number of sids is {sum2}')
            self.log(f'We use only {percent:.2f} % of the original sids thanks to merging duplicated operations.')
    ...
    def compare_cluster_centers_sum_after_merging_low_frequency(self, dict_before_merge, dict_after_mergeConflict,
        ...
        if self.accelerator.is_main_process:
            ...
            self.log(f"[statics2-inter] The original number of interaction is {sum_inter}")
            self.log(
            ...
            self.log(
                f'[Summary] We have {sum0} items, with a static tokenizer, we will use {sum0} sid to represent {sum0} items.'
                ...
                f'\n But with a cluster strategy, we will employ {sum1} sids to represent {sum0} items ({percent1} %, the reduced number compared to everyInteraction-a-sid: {sum_inter - sum1}'
                ...
                f'\n The total number of personalized semantic IDs will not remain excessively (e.g., to 500% sids of items), since most items are of low frequency with limited interactions,'
                f'\n which can not be assigned too many initial clusters. And Due to the merge infrequent strategy, a balance between personalization and sparsity is achieved.')
    ...
    def get_merging_conflict_searching_dict(self, itemID2sidTokenList):
        ...
        # statics
        if self.accelerator.is_main_process:
            self.log(f"The number of samples(itemID) in mergeConflictSearchingDict : {len(mergeConflictSearchingDict)}")
